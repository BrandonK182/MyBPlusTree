...
public:
    ...
    friend std::ostream& operator<<(std::ostream& outs, const set<T>& print_me) {
        print_me.print_tree(0, outs);
        return outs;
    }

    void Union(const set<T> rhs, set<T>& newTree);
    void UnionRecursive(const set<T> rhs, set<T>& newTree);
    void intersection(const set<T> rhs, set<T>& newTree);
