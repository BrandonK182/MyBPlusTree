...
public:
    ...
    void copy_tree(const BPlusTree<T>& other, BPlusTree<T>** last_node);      //copy other into this object
    ...
    friend std::ostream& operator<<(std::ostream& outs, const BPlusTree<T>& print_me) {
    ...
    *************************************************/
    ...
    void test_sample_const_get();

    //array function tests
    void test_sample_maximal();
    void test_sample_swapp();
    ...
    void test_transfer_right();
    void test_transfer_left();
    */
private:
...
template <class T>
void swapp(T& a, T& b);                                 //swap the two items
...
template <typename T>
std::vector<T> vector_instersection(std::vector<T>& v1, std::vector<T>& v2)
{
    std::vector<T> v3;
